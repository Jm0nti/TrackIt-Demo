...
function displayMessage(text, isError = false) {
  ...
  messageArea.className = `p-3 rounded-lg text-sm transition-all duration-300 ${
...
function renderShipment(data) {
  ...
  document.getElementById(
    "trackingIdDisplay"
  ).textContent = `ID: ${data.tracking_id}`;
  document.getElementById("normalizedStatus").textContent =
  ...
  document.getElementById("lastUpdate").textContent = formatBogotaTime(
    data.last_update
  );
  ...
  if (
    ...
    isFinite(Number(data.current_location.latitude)) &&
    isFinite(Number(data.current_location.longitude))
  ) {
  ...
  if (destinationEl)
    destinationEl.textContent = data.destination || "No disponible";
  ...
  if (
    ...
    const lat = Number(data.current_location.latitude);
    const lng = Number(data.current_location.longitude);
    const url = buildGoogleMapsEmbedUrl(lat, lng, 10);
    if (url && mapFrame) {
      mapFrame.src = url;
...
function buildGoogleMapsEmbedUrl(lat, lng, zoom = 15) {
  ...
  return `https://www.google.com/maps?q=${encodeURIComponent(
    nLat
  )},${encodeURIComponent(nLng)}&z=${zoom}&output=embed`;
}
...
async function fetchShipment(id, isUpdate = false) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        data.detail || `Error ${response.status}: Error desconocido`
      );
    }
